...
def scrapeNews(client, database, link):
    ...
    if link.startswith('https://leute'):
        listings = soup.find_all('section', attrs= {'class': 'tl-macher tl-snippet-content'})
        for listing in listings:
            for header in listing.find_all('h3', attrs = {'class': 'tl-h1'}):
                headline = header.get_text()
            for body in listing.find_all('div', attrs = {'class': 'col-xs-12 col-sm-8'}):
                text = body.get_text()
    else:
